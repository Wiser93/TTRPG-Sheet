  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <FormSection title="Identity">
        <LabeledInput label="Name" value={species.name ?? ''} onChange={e => patch({ name: e.target.value })} required placeholder="e.g. High Elf" />
        <LabeledTextarea label="Description" value={species.description ?? ''} rows={3}
          onChange={e => patch({ description: e.target.value })} placeholder="Brief description of this species…" />
      </FormSection>

      <FormSection title="Traits">
        <FormRow cols={3}>
          <LabeledSelect label="Size" value={species.size ?? 'medium'}
            onChange={e => patch({ size: e.target.value as Species['size'] })}
            options={['tiny','small','medium','large','huge','gargantuan'].map(s => ({ value: s, label: s.charAt(0).toUpperCase() + s.slice(1) }))} />
          <LabeledInput label="Walking Speed (ft)" type="number" min={0} step={5}
            value={species.speed ?? 30} onChange={e => patch({ speed: Number(e.target.value) })} />
          <LabeledInput label="Darkvision (ft, 0=none)" type="number" min={0} step={30}
            value={species.darkvision ?? 0} onChange={e => patch({ darkvision: Number(e.target.value) })} />
        </FormRow>

        {/* Extra speeds */}
        <div>
          <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase', color: 'var(--text-2)', marginBottom: 6 }}>
            Extra Speeds
          </p>
          {(species.extraSpeeds ?? []).map((es, idx) => (
            <div key={idx} style={{ display: 'flex', gap: 8, marginBottom: 6, alignItems: 'flex-end' }}>
              <div style={{ flex: 1 }}>
                <input value={es.type} onChange={e => updateExtraSpeed(idx, { type: e.target.value })} placeholder="Type (e.g. fly, swim, climb)" />
              </div>
              <div style={{ width: 80 }}>
                <input type="number" value={es.value} min={0} step={5}
                  onChange={e => updateExtraSpeed(idx, { value: Number(e.target.value) })} />
              </div>
      ...
      <FormSection title="Racial Features">
        <FeatureEditor features={species.features ?? []} onChange={features => patch({ features })} />
      </FormSection>
