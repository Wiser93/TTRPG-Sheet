        ...
        <div key={f.id} style={{ border: '1px solid var(--border)', borderRadius: 6, overflow: 'hidden' }}>
          ...
          {expandedId === f.id && (
            <div style={{ padding: 10, display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <UsesEditor
                uses={f.uses}
                onChange={uses => update(f.id, { uses })}
              />
              ...
              <div>
                ...
                <input
                  value={(f.tags ?? []).join(', ')}
                  onChange={e => update(f.id, { tags: e.target.value.split(',').map(t => t.trim()).filter(Boolean) })}
...
function UsesEditor({
  ...
  uses?: Feature['uses'];
  onChange: (uses: Feature['uses']) => void;
}) {
  const hasUses = uses !== undefined;
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        <div>
          ...
          <input
            type="number"
            min={1}
            value={uses.max.type === 'flat' ? uses.max.value : ''}
            onChange={e => onChange({ ...uses, max: { type: 'flat', value: Number(e.target.value) } })}
            placeholder="e.g. 3"
          />
        </div>
        <div>
          ...
          <select
            ...
            onChange={e => onChange({ ...uses, rechargeOn: e.target.value as Feature['uses']['rechargeOn'] })}
          >
