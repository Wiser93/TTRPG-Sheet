...
import type { Item, ItemCategory, Rarity, DamageType, WeaponStats } from '@/types/game';
...
export function ItemForm({ initial, onSave, onCancel, isSaving }: ItemFormProps) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    onSave({
      ...
      weaponStats: showWeapon ? defaultWeapon(item.weaponStats) : undefined,
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <FormSection title="Weapon Stats">
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, cursor: 'pointer' }}>
          <input type="checkbox" checked={showWeapon} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setShowWeapon(e.target.checked)}
            style={{ accentColor: 'var(--accent)', width: 15, height: 15 }} />
        ...
        {showWeapon && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingLeft: 8, borderLeft: '2px solid var(--border)' }}>
            <FormRow cols={3}>
              ...
              <LabeledInput label="Bonus" type="number" value={weapon.damage.modifier}
            ...
            <TagInput label="Weapon Properties" values={weapon.properties} suggestions={WEAPON_PROPERTIES}
              onChange={(props: string[]) => patch({ weaponStats: { ...weapon, properties: props } })} />
      ...
      <FormSection title="Armor Stats">
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, cursor: 'pointer' }}>
          <input type="checkbox" checked={showArmor} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setShowArmor(e.target.checked)}
            style={{ accentColor: 'var(--accent)', width: 15, height: 15 }} />
        ...
        {showArmor && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingLeft: 8, borderLeft: '2px solid var(--border)' }}>
            <FormRow cols={2}>
              ...
              <LabeledInput label="Max DEX Bonus (-1 = unlimited)" type="number" min={-1} value={item.armorStats?.maxDexBonus ?? -1}
            ...
            <FormRow>
              <LabeledInput label="STR Requirement" type="number" min={0} value={item.armorStats?.strengthRequired ?? 0}
