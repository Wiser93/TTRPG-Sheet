...
export function FeatForm({ initial, onSave, onCancel, isSaving }: FeatFormProps) {
  const [feat, setFeat] = useState<Partial<Feat>>(() => ({
    name: '', description: '', prerequisites: '', repeatable: false, features: [], choices: [],
    ...initial,
  }));
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <FormSection title="Identity">
        <LabeledInput label="Name" value={feat.name ?? ''} required placeholder="e.g. Alert"
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => patch({ name: e.target.value })} />
        <LabeledTextarea label="Description" value={feat.description ?? ''} rows={4} required placeholder="What does this feat do?"
          onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => patch({ description: e.target.value })} />
        <LabeledInput label="Prerequisites" value={feat.prerequisites ?? ''} placeholder="e.g. Spellcasting feature (optional)"
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => patch({ prerequisites: e.target.value })} />
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, cursor: 'pointer' }}>
          <input type="checkbox" checked={feat.repeatable ?? false}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => patch({ repeatable: e.target.checked })}
      ...
      <FormSection title="Features">
        <FeatureEditor features={feat.features ?? []}
